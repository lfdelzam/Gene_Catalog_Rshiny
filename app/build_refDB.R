...
path_to_taxonomy_db <- paste(directorio_db,"rep_genes_taxonomy.tsv", sep="/")
...
path_to_clusters <- paste(directorio_db,"mix_cluster_ind_in_co_reps.tsv", sep="/")


KEEG_list<-function(x ,sep){
  my_list <- str_split(x, sep)%>% unlist()
  element<-my_list[grep("[K]\\d",  my_list)]
  text <- gsub("ko:","",element) # [1] change webapp so several KEGG are possible
  if (length(text) > 0) {
...
if (file.exists(path_to_clusters) & !exists("cluster_df") ) {
  cluster_df<-read_tsv(path_to_clusters, col_names = c("co", "ind"), num_threads = C_pus, show_col_types = FALSE)
